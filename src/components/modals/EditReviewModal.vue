...
const hasChanges = computed(() => {
  return rating.value !== props.review.rating || 
         comment.value.trim() !== props.review.comment.trim();
});
...
const submitUpdate = async () => {
  ...
  try {
    const success = await reviewStore.updateReview(props.review.id, {
      rating: rating.value,
    ...
    if (success) {
      ...
      errorMessage.value = reviewStore.error || 'Failed to update review';
    }
  } catch (err) {
    errorMessage.value = err instanceof Error ? err.message : 'Failed to update review';
  } finally {
      ...
      <div class="modal-body">
        ...
        <div class="comment-section">
          ...
          <textarea
            ...
            :disabled="submitting"
          ></textarea>
          <p class="character-count" :class="{ 'text-danger': comment.length < 10 && comment.length > 0 }">
        ...
        <!-- Info Message -->
        <div v-if="!hasChanges && isValid" class="info-message">
          No changes detected
        </div>
